...
All of the hosts in the cluster need to be resolveable via DNS.  In order to test that this is working execute the following from any host in your cluster to each other host in your cluster.   Additionally if using a control node it too should be able to resolve all hosts in your cluster. Running the following with the targeted host names and the IP address of you DNS server should confirm resolution. 

`$ dig <node_hostname> @<IP_address> +short`


In and HA cluster there will also be a DNS name for the Load Balanced URL that points to the 3 master servers for access to the API, CLI and Console services.  This value should also resolve, and will be placed in the ansible Hosts file for the variables. 
...
In addition to the hostnames for the master console and API, A wildcard DNS entry needs to exist under a unique subdomain (i.e. `*.cloudapps.c1-ocp.myorg.com`) that resolves to either the IP addresses (an A record) or the hostnames (a CNAME record) of the three Infrastructure Nodes.  
...
image::/images/dnsmastersinfra.png[DNS Diagram]
